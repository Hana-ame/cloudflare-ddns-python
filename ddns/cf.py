...
def gen_A(name="@",ip="127.0.0.1", ttl=1):
    return {
        "type":"A",
        "name": name,
        "content": ip,
...
def createRecord(name,ip,token=TOKEN,ttl=1):
    data = gen_A(name,ip,ttl)
    if len(ip.split(':'))>1: # IPv6
        data = gen_AAAA(name,ip,ttl)
    pl = json.dumps(data)
    pl = pl.encode()
    req = urllib.request.Request(
        'https://api.cloudflare.com/client/v4/zones/' + ZONE_ID + '/dns_records',
...
def updateRecord(name,ip,id,token=TOKEN,ttl=1):
    data = gen_A(name,ip,ttl)
    if len(ip.split(':'))>1: # IPv6
        data = gen_AAAA(name,ip,ttl)
    pl = json.dumps(data)
